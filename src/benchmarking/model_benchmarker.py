...
class ModelBenchmarker:
    ...
    def analyze_skill_extraction(self):
        ...
        for model_name, scores in self.results.items():
            all_skills = []
            for score in scores:
                all_skills.extend([skill['compétence'] for skill in score['top_skills']])
    ...
    def save_results(self, output_file):
        with open(output_file, 'w', newline='', encoding='utf-8') as file:
            ...
            writer.writerow(['Model', 'CV', 'Job Description', 'Model Score', 'Manual Score', 'Top Skills'])
            for model, scores in self.results.items():
                for score in scores:
                    writer.writerow([
                        ...
                        score['model_score'],
                        score['manual_score'],
                        '; '.join([f"{skill['compétence']} ({skill['score']:.2f})" for skill in score['top_skills']])
                    ])
