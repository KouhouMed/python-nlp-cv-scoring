...
class Word2VecScorer(BaseScorer):
    def __init__(self):
        ...
        model_name = self.config.get('models.word2vec.model_name', 'fasttext-wiki-news-subwords-300')
    ...
    def score_cv(self, cv_text, job_description):
        ...
        experience_multiplier = min(1 + (total_experience / max_experience_years),
                                    self.config.get('skill_scoring.experience_multiplier_cap', 2.0))

        # Apply certification boost
        cert_boost = 1 + (len(certifications) * self.config.get('skill_scoring.certification_boost', 0.1))

        # Apply project relevance boost
        project_vectors = [self.get_document_vector(project) for project in projects]
        project_relevance = np.mean([self.cosine_sim(proj_vec, job_vector) for proj_vec in project_vectors]) if projects else 0
        project_boost = 1 + (project_relevance * self.config.get('skill_scoring.project_relevance_boost_cap', 0.2))
    ...
    def extract_top_skills(self, cv_text, job_description):
        ...
        for skill, years in skills.items():
            ...
            # Apply experience multiplier
            max_skill_experience = self.config.get('skill_scoring.max_skill_experience', 10)
            experience_multiplier = min(1 + (years / max_skill_experience),
                                        self.config.get('skill_scoring.experience_multiplier_cap', 2.0))

            # Apply certification boost
            cert_boost = self.config.get('skill_scoring.certification_boost', 1.2) if any(
                cert.lower() in skill.lower() for cert in certifications) else 1.0
            ...
            for project in projects:
                ...
                if skill.lower() in project.lower():
                    project_boost = max(project_boost, 1 + (
                                project_relevance * self.config.get('skill_scoring.project_relevance_boost_cap', 0.2)))

            # Calculate final score
            final_score = base_score * experience_multiplier * cert_boost * project_boost
        ...
        top_skills = sorted(skill_scores.values(), key=lambda x: x["score"], reverse=True)[:top_skills_count]
        return top_skills